...
class SequentialCIFAR100(ContinualDataset):
    ...
    # -------------------- ADDED: helpers used by base models/eval --------------------
    def get_task_labels(self, task_idx: int) -> List[int]:
        """
        Return the class indices that belong to task `task_idx`
        (standard contiguous split: 10 classes per task).
        """
    ...
    def get_current_labels(self) -> List[int]:
        """
        Return labels of the *current* task based on the internal pointer `self.i`
        set by `store_masked_loaders`. Falls back to task 0 if not set.
        """
        task_idx = getattr(self, 'i', 0)
        return self.get_task_labels(task_idx)
    # -------------------------------------------------------------------------------
    ...
    def get_scheduler(model, args) -> torch.optim.lr_scheduler:
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(model.opt, [35, 45], gamma=0.1)
        return scheduler
