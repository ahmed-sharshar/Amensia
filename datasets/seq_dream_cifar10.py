...
from datasets.seq_cifar10 import MyCIFAR10, SequentialCIFAR10
...
class DreamDataset(MyCIFAR10):

    DREAM_CLASS_LABELS = []
    DREAM_CLASS_NAMES = []

    def __init__(self, root_1:str, root_2:str, train:bool = True, transform:transforms = None, target_transform: transforms =None, 
                 download:bool=False, labels=list(range(SequentialCIFAR10.N_CLASSES_PER_TASK)) + [None] * SequentialCIFAR10.N_CLASSES_PER_TASK * (SequentialCIFAR10.N_TASKS - 1)) -> None:
...
class DreamCIFAR10(SequentialCIFAR10):
    NAME = 'dream-seq-cifar10'
    ...
    def get_data_loaders(self):
        ...
        train_dataset = DreamDataset(base_path() + 'CIFAR10', base_path() + self.args.dataset_2, train=True, download=True, transform=transform, labels = list(self.pos_to_label.values()))
        test_dataset = DreamDataset(base_path() + 'CIFAR10', base_path() + self.args.dataset_2, train=False, download=True, transform=test_transform, labels = list(self.pos_to_label.values()))
